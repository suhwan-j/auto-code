# TOTORO-CODE: Advanced CLI Agent
...
> **CLI**: `totoro`  
> **Package**: `totoro-agent` (directory: `totoro/`)
...
### DeepAgents가 제공하는 것 vs TOTORO-CODE가 추가하는 것
...
├─────────────────────────────────────────────────────────────┤
│  TOTORO-CODE Extensions (on top of DeepAgents)                  │
...
agent = create_deep_agent(
    ...
    # Only specify TOTORO-CODE custom tools here.
    ...
    # Skills directories
    skills=["built-in/skills/", "~/.totoro/skills/", ".totoro/skills/"],
...
### 색상 팔레트 (totoro/colors.py)

Totoro Code는 truecolor ANSI 기반의 전용 색상 팔레트를 사용한다.

| 역할 | 색상 | Hex | 용도 |
|------|------|-----|------|
| **Blue** | 파랑 | `#56A0F0` | 프롬프트, 링크 |
| **Amber** | 호박 | `#F5B240` | 강조, 로고 |
| **Copper** | 구리 | `#E06840` | 경고/에러 전용 |
| **Ivory** | 아이보리 | `#D4BA8E` | 보조 텍스트 |
...
`create_deep_agent()`가 자동으로 포함하는 도구. TOTORO-CODE에서 별도 등록 불필요.
...
### 3.2 TOTORO-CODE 커스텀 도구

TOTORO-CODE가 `tools=[]` 파라미터로 추가하는 CLI 전용 도구.
...
Git은 코딩 에이전트의 핵심 워크플로이므로 TOTORO-CODE 전용 도구로 제공한다.
...
bash 도구는 TOTORO-CODE 커스텀 도구이며 선택적 샌드박스 모드를 지원한다.
...
설정:
{
  "sandbox": {
    ...
    "container_image": "totoro-code-sandbox:latest"
  }
}
```
...
```
[1] DeepAgents 내장 도구 + TOTORO-CODE 커스텀 도구 (항상 우선)
[2] ~/.totoro/.mcp.json (사용자 전역)
[3] .totoro/.mcp.json (프로젝트)
[4] .mcp.json (프로젝트 루트)
...
TOTORO-CODE는 DeepAgents의 `interrupt_on` + `Command(resume)` 메커니즘을 그대로 사용한다.
...
DeepAgents 미들웨어 훅 포인트를 통해 TOTORO-CODE 커스텀 레이어를 에이전트 실행 루프에 삽입한다.
...
```python
# create_totoro_agent() 내부에서 미들웨어로 통합
agent = create_deep_agent(
    ...
    middleware=[custom_middleware],  # TOTORO-CODE 커스텀 레이어
)
```
...
DeepAgents의 Backend 시스템과 Store를 조합하고, TOTORO-CODE가 Auto-Dream 추출 레이어를 추가한다.
...
### 11.4 자동 메모리 추출 (Auto-Dream) — TOTORO-CODE 확장

DeepAgents의 MemoryMiddleware 위에 TOTORO-CODE가 추가하는 자동 추출 레이어.
...
TOTORO-CODE는 `create_deep_agent()`의 `skills` 파라미터를 그대로 사용한다.
...
[1] CLI 인수              (최우선)
[2] 환경 변수
[3] .totoro/settings.json  (프로젝트)
[4] ~/.totoro/settings.json  (사용자)
[5] 기본값                (최하위)
```
...
```
┌──────────────────────────────────────────────────────────┐
│  TOTORO-CODE CLI/TUI Layer                                     │
│  ├── Textual TUI                                          │
│  ├── Slash Commands                                       │
│  ├── Session Management                                   │
│  └── Stall Detection                                      │
├──────────────────────────────────────────────────────────┤
│  TOTORO-CODE Custom Tools                                      │
│  ├── git (+ safety rules)                                 │
│  ├── bash (+ sandbox)                                     │
│  ├── web_search (Tavily)                                  │
│  ├── fetch_url                                            │
